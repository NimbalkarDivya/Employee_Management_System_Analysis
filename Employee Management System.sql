...
SELECT jd.jobdept, AVG(sb.amount) AS avg_salary_range
FROM SalaryBonus sb
JOIN JobDepartment jd ON sb.Job_ID = jd.Job_ID
GROUP BY jd.jobdept;
...
SELECT COUNT(DISTINCT q.Emp_ID) AS employees_with_qualification
...
SELECT YEAR(date) AS leave_year, COUNT(DISTINCT emp_ID) AS employees_took_leave
FROM Leaves
GROUP BY leave_year
ORDER BY employees_took_leave DESC
LIMIT 1;

-- 4.2 What is the average number of leave days per department?
SELECT jd.jobdept, AVG(leave_count) AS avg_leave_days
FROM (
...
-- 4.4 What is the total number of leave days company-wide?
...
-- 5.4 What is the average total_amount after considering leave deductions?
